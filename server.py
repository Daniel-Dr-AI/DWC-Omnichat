...
import os, logging, datetime, sqlite3, asyncio, json
from logging.handlers import RotatingFileHandler
from pathlib import Path
from typing import Optional, Dict, Set
import sys
print(">>> LOADED server.py FROM:", __file__, file=sys.stderr)
...
async def startup_tasks():
    ...
    # kick off background escalation loop
...
async def ws_endpoint(websocket: WebSocket, user_id: str):
    ...
    try:
        while True:
            # Receive text frames; widget will send small JSON for typing presence
            msg = await websocket.receive_text()
            try:
                data = json.loads(msg)
            except Exception:
                continue  # ignore non-JSON heartbeats/noise

            ev_type = (data.get("type") or "").lower()
            if ev_type in ("typing", "stop_typing"):
                # Relay presence to admin dashboards (no DB write, no echo to visitor)
            ...
            # else: ignore any other client-side signals
...
async def ws_admin(websocket: WebSocket):
    ...
    # Immediately replay open conversations + messages from DB
    try:
        with db() as conn:
            c = conn.cursor()
...
DEBUG_ADMIN_PUSH = True  # keep True while testing
...
async def push_with_admin(user_id: str, channel: str, payload: dict):
    ...
    # Echo to the visitor only if it's not a 'user' echo (avoid mirroring their own text)
    ...
    for ws in list(admin_connections):
        try:
            ...
            # prune dead/errored sockets
            if ws in admin_connections:
                admin_connections.remove(ws)
    if DEBUG_ADMIN_PUSH:
        logging.info("[DEBUG] push_with_admin: sent to %d admin client(s)", sent)
